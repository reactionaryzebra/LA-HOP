...
class StatusBase extends React.Component {
    ...
    seeStatus=(num)=>{
        console.log(num,'<----num in see Status')
        const {firebase, history}=this.props
        const here = this
        firebase.db.collection('requests').doc(num.toString()).get().then(function(docRef) {
    ...
    render() {
      ...
      return (
        ...
        <button onClick={()=>{this.seeStatus(document.getElementById("uniqueStatus").value)}}>See status</button>
        {this.state.personStatus?<h4>{this.state.personStatus}</h4>:undefined}
        </div>
      );
    }
  }
